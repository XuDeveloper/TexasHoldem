  ...
  container.innerHTML = `
    <div class="room-container animate-fade-in">
      <div class="room-header">
        <h2 class="room-title">🃏 等待大厅</h2>
        <div class="room-code-display">
          <span class="room-code-label">房间代码</span>
          <span id="room-code-value" class="room-code-value">${room.id}</span>
          <button id="btn-copy" class="btn-icon" title="复制代码">📋</button>
        </div>
      </div>

      <div class="room-body panel">
        <div class="players-section">
          <h3>玩家 <span id="player-count">(${room.players.length}/9)</span></h3>
          <div id="player-list" class="player-list"></div>
        </div>

        <div id="host-controls" class="host-controls ${amHost ? '' : 'hidden'}">
          <button id="btn-add-ai" class="btn btn-secondary">🤖 添加 AI</button>
          <button id="btn-start" class="btn btn-primary" ${room.players.length < 2 ? 'disabled' : ''}>▶ 开始游戏</button>
        </div>

        <div class="chat-section">
          <div id="chat-messages" class="chat-messages"></div>
          <div class="chat-input-row">
            <input type="text" id="chat-input" class="input chat-input" placeholder="输入消息..." maxlength="200">
            <button id="btn-send" class="btn btn-secondary btn-small">发送</button>
          </div>
        </div>
      </div>

      <button id="btn-leave" class="btn btn-danger btn-small">离开房间</button>
    </div>
  `;
  ...
  function renderPlayers(players) {
    ...
    list.innerHTML = players.map(p => `
      <div class="player-card ${!p.isConnected ? 'disconnected' : ''} ${p.id === currentRoom.hostId ? 'is-host' : ''}">
        ...
        ${!p.isConnected ? '<span class="player-status text-red">离线</span>' : ''}
      </div>
    `).join('');
  }
});
